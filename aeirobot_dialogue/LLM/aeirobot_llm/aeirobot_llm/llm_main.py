...
from aeirobot_llm.agents.llm import LangchainLLM
...
class LlmGPTNode(Node):
    def __init__(self):
        ...
        if not config_file:
            config_file = os.path.join(current_directory, '..', 'config', 'llm_config.yaml')
        ...
        self.pub_llm = self.create_publisher(
            String, self.config["ros__parameters"]["pub"]["pub_llm"], 10)
        
        self.pub_llm_full = self.create_publisher(
            String, self.config["ros__parameters"]["pub"]["pub_llm_full"], 10)

        self.sub_llm_clear = self.create_subscription(
            Bool, self.config["ros__parameters"]["sub"]["sub_llm_clear"], 
            self.llm_clear_callback, 10)

        self.sub_stt = self.create_subscription(
            String, self.config["ros__parameters"]["sub"]["sub_stt"], 
            self.stt_callback, 10
            )

        # New subscriber for process signal
        self.sub_llm_process = self.create_subscription(
            Bool, self.config["ros__parameters"]["sub"]["sub_llm_process"], 
            self.llm_process_callback, 10
            )

        self.sub_initialize = self.create_subscription(
            Bool, self.config["ros__parameters"]["sub"]["sub_llm_initialize"], 
            self.initialize_callback, 10
            )

        
        # LangchainLLM object initialization (도구 없는 순수 LLM)
        self.langchain = LangchainLLM(
            publisher=self.pub_llm, 
            publisher_tools=None,  # LLM은 도구를 사용하지 않음
...
# ----------------------------------------------------------
    ...
    def initialize_callback(self, msg):
        if msg.data:
            ...
            self.latest_stt_input = None
            self.get_logger().info(f'LLM node initialized with new user ID: {self.current_user_id}')
