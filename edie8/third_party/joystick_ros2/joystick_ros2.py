...
JOYSTICK_CODE_VALUE_MAP = {
    ...
    # 'Logitech Logitech Cordless RumblePad 2': (F710_CODE_MAP, F710_VALUE_MAP),
...
class JoystickRos2(Node):
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            # print('before while True:',time.time())
            # read inputs from joystick
            while True:
                # print('when while true',time.time())
                try:
                ...
                if events:
                    for event in events:
                        if (event.code in JOYSTICK_CODE_VALUE_MAP[event.device.name][0]):
                            ...
                            # print('when event in events:',time.time())
                            ...
                            if (event.ev_type == 'Key'):
                                ...
                                self.last_event = event
                #             else :
                #                 print('when here in 2',time.time())
                #         else:
                #             print('when here in 1',time.time())        
                else:
                    # print('when else',time.time())
                    time.sleep(0.005)
                    self.publish_joy()
                    # self.publish_joy()
                    # break
            ## autorepeat
            # print('when autorepeat',time.time())
