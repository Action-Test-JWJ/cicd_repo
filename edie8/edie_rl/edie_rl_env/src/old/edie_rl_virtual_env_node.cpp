#include <edie_rl_env/edie_rl_virtual_env_node.h>
...
//######################################################################################
Dummy::Dummy(int state_number_, int action_number_, int state_sector_number_)
{
...
Dummy::~Dummy(){}
//######################################################################################
EDIE::EDIE_reinforcement_env::EDIE_reinforcement_env() : Node("edie_rl_env"){
    state_number            =   9;//9;
    action_number           =   15;//15;
    ...
    dummy_instance          =   new Dummy(state_number, action_number, state_sector_number);
...
bool EDIE::EDIE_reinforcement_env::ActionServiceCallback(std::shared_ptr<edie_msgs::srv::Reinforcement::Request> req, 
    ...
    for(int i = 0; i<state_number-2; i++)
    {
        for(int j = 0; j<state_sector_number; j++){
            if(input_state.at(i) >= sector_interval*j && input_state.at(i) <= sector_interval*(j+1)){
                state_emotion[i] = input_state.at(i) + dummy_instance->personality_table[state_sector_number*i+j][max_action_index]*table_value_chanegr ;
                if(state_emotion[i]<0) state_emotion[i]=0;
                std::cout <<  "state at " << i <<": " << input_state.at(i) << " + " << dummy_instance->personality_table[state_sector_number*i+j][max_action_index]*table_value_chanegr  << " = " << state_emotion[i] << std::endl; 
            }
        }
    }
    for(int i = 0; i < state_number-2; i++){
    ...
    for(int i = 0; i < state_number-2; i++){
        if(i == 1 || i == 6){
            if(reaction_state[i]-input_state[i] > 0)        reward += (reaction_state[i]-input_state[i]) * 2;
            else if(reaction_state[i]-input_state[i] < 0)   reward += (reaction_state[i]-input_state[i]) * 1;
            else if(reaction_state[i]-input_state[i] == 0){
                if(i == max_next_state_index) reward += 0.01;
                else reward += 0;
            }
...
bool EDIE::EDIE_reinforcement_env::DoneServiceCallback(std::shared_ptr<edie_msgs::srv::Reinforcement::Request> req, 
                                                            std::shared_ptr<edie_msgs::srv::Reinforcement::Response> res){
