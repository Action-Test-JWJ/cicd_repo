...
#include "edie_passthrough_controller/passthrough_controller.hpp"
#include "controller_interface/helpers.hpp"
#include "pluginlib/class_list_macros.hpp"

namespace edie_passthrough_controller
{
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  for (size_t i = 0; i < command_interfaces_.size(); ++i)
  {
    if (!std::isnan(reference_interfaces_[i]))
    {
      ...
      //   "reference_interfaces: " << reference_interfaces_[0]);
    }
  }
...
controller_interface::return_type PassthroughController::update_reference_from_subscribers()
{
  ...
  if (!(!joint_commands || !(*joint_commands)))
  {
    ...
    // RCLCPP_INFO(
    //   get_node()->get_logger(),
    //   "Received command update from topic (size=%zu): [ %s ]",
    //   (*joint_commands)->data.size(),
    //   fmt::format(
    //     "{}",
    //     fmt::join((*joint_commands)->data, ", ")
    //   ).c_str()
    // );
    reference_interfaces_ = (*joint_commands)->data;

    rt_buffer_ptr_.writeFromNonRT(nullptr);
  }
...
PLUGINLIB_EXPORT_CLASS(
  edie_passthrough_controller::PassthroughController, controller_interface::ChainableControllerInterface)
