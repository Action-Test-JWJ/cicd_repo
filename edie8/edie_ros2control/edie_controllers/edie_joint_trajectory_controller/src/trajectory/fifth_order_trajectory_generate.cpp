...
FifthOrderTrajectoryGenerator::FifthOrderTrajectoryGenerator()
{
...
bool FifthOrderTrajectoryGenerator::DetectChangeFinalValue(double pose_, double velocity_, double time_)
{
...
double FifthOrderTrajectoryGenerator::GenerateFifthOderTrajectory(double initial_value_, double final_value_,
...
double FifthOrderTrajectoryGenerator::GenerateOneFifthOderTrajectory(Eigen::MatrixXd joint_)
{
	double result_one_joint_;

	if (DetectChangeFinalValue(joint_(0, 1), joint_(0, 3), joint_(0, 7)))
	{
		current_time = 0;
		// ROS_INFO("One Value Change!");
	}
	result_one_joint_ = GenerateFifthOderTrajectory(current_pose, joint_(0, 1), current_velocity, joint_(0, 3), joint_(0, 4), joint_(0, 5), joint_(0, 6), joint_(0, 7));
