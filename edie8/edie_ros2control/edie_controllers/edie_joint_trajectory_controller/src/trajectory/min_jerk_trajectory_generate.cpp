#include "edie_joint_trajectory_controller/trajectory/min_jerk_trajectory_generate.hpp"

MinimumJerkTrajectoryGenerator::MinimumJerkTrajectoryGenerator(double ini_time, double fin_time,
...
std::vector<double_t> MinimumJerkTrajectoryGenerator::getPosition(double time)
{
  if(time >= fin_time_)
    cur_pos_  = fin_pos_;
...
std::vector<double_t> MinimumJerkTrajectoryGenerator::getVelocity(double time)
{
  if(time >= fin_time_)
    cur_vel_  = fin_vel_;
  else if(time <= ini_time_)
...
std::vector<double_t> MinimumJerkTrajectoryGenerator::getAcceleration(double time)
{
  if(time >= fin_time_)
    cur_acc_  = fin_acc_;
  else if(time <= ini_time_)
