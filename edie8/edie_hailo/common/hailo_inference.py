    ...
    def run(self) -> None:
        ...
        with self.infer_model.configure() as configured_infer_model:
            while True:
                ...
                # print(f'[HailoAsyncInference] run: batch_data: {batch_data}')
                # print('---------------------------------------------\n')
                ...
                # Support tuples (frame, meta) in batches for latency measurement
                frames_data = []
                metas = []
                for item in preprocessed_batch:
                    if isinstance(item, tuple) and len(item) == 2:
                        frame_data, meta = item
                ...
                for m in metas:
                    m['t_infer_start'] = t_infer_start

                job = configured_infer_model.run_async(
                    bindings_list,
                    partial(
                        self.callback_fn,
                        input_batch=[(frames_data[i], metas[i]) for i in range(len(frames_data))],
                        bindings_list=bindings_list,
                    )
                )

            job.wait(10000)  # Wait for the last job [10초]
