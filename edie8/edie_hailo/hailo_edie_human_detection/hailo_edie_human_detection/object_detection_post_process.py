...
def inference_result_handler(original_frame, infer_results, labels, config_data, tracker=None):
    ...
    Returns:
        Tuple[np.ndarray, list[dict]]: (annotated_frame, detections_list)
        detections_list item schema: {
            'cls_id': int,
            'label': str,
            'score': float,
            'bbox': [x, y, w, h]
    ...
    """
    det_dict = extract_detections(original_frame, infer_results, config_data)
    frame_with_detections = draw_detections(det_dict, original_frame, labels, tracker=tracker)

    # Convert detection dict → list[dict] that the node expects
    boxes = det_dict.get('detection_boxes', [])        # [ymin, xmin, ymax, xmax]
    scores = det_dict.get('detection_scores', [])
    classes = det_dict.get('detection_classes', [])
    num = det_dict.get('num_detections', 0)
    ...
    for i in range(num):
        try:
            ...
            detections_list.append({
                ...
                'label': labels[int(classes[i])] if i < len(classes) else '',
    ...
    return frame_with_detections, detections_list
...
def draw_detections(detections: dict, img_out: np.ndarray, labels, tracker=None):
    ...
    # print(f'[draw_detections] boxes: {boxes}')
    # print(f'[draw_detections] scores: {scores}')
    # print(f'[draw_detections] num_detections: {num_detections}')
    # print(f'[draw_detections] classes: {classes}')
    # print('---------------------------------------------\n')
