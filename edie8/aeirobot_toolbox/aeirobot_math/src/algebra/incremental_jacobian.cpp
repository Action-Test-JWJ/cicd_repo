#include "aeirobot_math/algebra/incremental_jacobian.hpp"
#include <cstdlib>
...
IncrementalJacobian::IncrementalJacobian(int numjoint, int delta_time)
{
    ...
    bodies[0].jacobian_matrix = Eigen::Matrix<double, 6, Eigen::Dynamic>::Zero(6, numjoint_);
    bodies[0].derivative_jacobian_matrix = Eigen::Matrix<double, 6, Eigen::Dynamic>::Zero(6, numjoint_);
    bodies[0].current_body_velocity = Eigen::Matrix<double, 6, 1>::Zero();
    bodies[0].current_body_acceleration = Eigen::Matrix<double, 6, 1>::Zero();
    bodies[0].select_joint_number_matrix = Eigen::Matrix<double, Eigen::Dynamic, 1>::Zero(numjoint_, 1);
    ...
    for (int i = 1; i <= numjoint_; ++i)
    {
        ...
        bodies[i].jacobian_matrix = Eigen::Matrix<double, 6, Eigen::Dynamic>::Zero(6, numjoint_);
        bodies[i].derivative_jacobian_matrix = Eigen::Matrix<double, 6, Eigen::Dynamic>::Zero(6, numjoint_);
        bodies[i].select_joint_number_matrix = Eigen::Matrix<double, Eigen::Dynamic, 1>::Zero(numjoint_, 1);
        ...
        bodies[i].between_twist_prev_body = Eigen::Matrix<double, 6, 1>::Zero();

        // bodies[i].joint_matrix_e = Eigen::MatrixXd::Zero(6, 1); // Adjust based on joint type
        bodies[i].joint_matrix_e = Eigen::Matrix<double, 6, 1>::Zero();
        ...
        bodies[i].current_body_velocity = Eigen::Matrix<double, 6, 1>::Zero();
        bodies[i].current_body_acceleration = Eigen::Matrix<double, 6, 1>::Zero();
    }
}
