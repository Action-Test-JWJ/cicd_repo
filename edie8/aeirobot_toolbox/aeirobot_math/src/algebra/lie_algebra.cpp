...
namespace aeirobot
{
    ...
    Eigen::Matrix4d LieAlgebra::Ceil6DVectorOperator(const Eigen::Matrix<double, 6, 1> &V)
    {
        Eigen::Matrix4d M;
        M << 0, -V(5), V(4), V(0),
            V(5), 0, -V(3), V(1),
    ...
    Eigen::Matrix<double, 6, 1> LieAlgebra::Floor6DVectorOperator(const Eigen::Matrix4d &M)
    {
        // Eigen::VectorXd V(6);
    ...
    Eigen::Matrix<double, 6, 6> LieAlgebra::AdjointTransform(const Eigen::Matrix4d &T)
    {
    ...
    Eigen::Matrix<double, 6, 6> LieAlgebra::AdjointTransformTranspose(const Eigen::Matrix4d &T)
    {
    ...
    // Eigen::Matrix<double, 6, 6> LieAlgebra::AdjointTransformInverseTranspose(const Eigen::Matrix4d &T)
    ...
    Eigen::Matrix<double, 6, 6> LieAlgebra::AdOperator(const Eigen::Matrix<double, 6, 1> &twist)
    {
        ...
        Eigen::Matrix<double, 6, 6> ad_matrix;
        ad_matrix.setZero();
    ...
    Eigen::Matrix<double, 6, 6> LieAlgebra::CoAdOperator(const Eigen::VectorXd &twist)
    {
        Eigen::Vector3d v = twist.head<3>();
        Eigen::Vector3d omega = twist.tail<3>();
        Eigen::Matrix3d omega_cross = Ceil3DVectorOperator(omega);
    ...
    Eigen::Matrix<double, 6, 6> LieAlgebra::InverseAdjoint(const Eigen::Matrix4d &T)
    {
        Eigen::Matrix3d R = T.block<3, 3>(0, 0);           // 3x3 회전 행렬
    ...
    Eigen::Matrix<double, 6, 6> LieAlgebra::CreateAdjointMatrix(const Eigen::Matrix<double, 6, 1> &V)
    {
        assert(V.size() == 6);
