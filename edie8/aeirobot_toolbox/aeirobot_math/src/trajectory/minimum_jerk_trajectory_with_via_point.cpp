#include "aeirobot_math/trajectory/minimum_jerk_trajectory_with_via_point.hpp"
...
namespace aeirobot
{
  MinimumJerkViaPoint::MinimumJerkViaPoint(double ini_time, double fin_time, double via_time, double ratio,
                                           ...
                                           std::vector<double_t> via_pos, [[maybe_unused]] std::vector<double_t> via_vel, [[maybe_unused]] std::vector<double_t> via_acc)
  {
    ratio_ = ratio;
    ...
    if (fin_time_ > ini_time_)
    {
      ...
      // time_mat.resize(7, 7);
      Eigen::Matrix<double, 7, 7> time_mat;
      ...
      Eigen::Matrix<double, 7, 1> conditions_mat;
      conditions_mat.setZero();
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        Eigen::Matrix<double, 7, 1> position_coeff = time_mat.inverse() * conditions_mat;
  ...
  std::vector<double_t> MinimumJerkViaPoint::getPosition(double time)
  {
    ...
    else if (time > ini_time_ && time <= via_time_)
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        double cur_pos = (time_variables_ * position_coeff_.block(0, i, 7, 1)).coeff(0, 0);
        cur_pos_[i] = cur_pos;
      }
    }
  ...
  std::vector<double_t> MinimumJerkViaPoint::getVelocity(double time)
  {
    ...
    else if (time > ini_time_ && time <= via_time_)
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        double cur_vel = (time_variables_ * velocity_coeff_.block(0, i, 7, 1)).coeff(0, 0);
        cur_vel_[i] = cur_vel;
      }
    }
    else if (time < fin_time_ && time > via_time_)
  ...
  std::vector<double_t> MinimumJerkViaPoint::getAcceleration(double time)
  {
    ...
    else if (time > ini_time_ && time <= via_time_)
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        double cur_acc = (time_variables_ * acceleration_coeff_.block(0, i, 7, 1)).coeff(0, 0);
        cur_acc_[i] = cur_acc;
      }
    }
    else if (time < fin_time_ && time > via_time_)
