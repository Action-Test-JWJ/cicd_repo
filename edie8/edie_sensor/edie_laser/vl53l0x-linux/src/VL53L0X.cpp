...
VL53L0X::VL53L0X(const std::string dev, const int16_t xshutGPIOPin, bool ioMode2v8, const uint8_t address) {
	this->dev = dev;
...
void VL53L0X::writeRegister(uint8_t reg, uint8_t value) {
	bool p = I2Cdev::writeByte(dev, this->address, reg, value);
	if (!p) {
...
void VL53L0X::writeRegister16Bit(uint8_t reg, uint16_t value) {
	...
	bool p = I2Cdev::writeWord(dev, this->address, reg, value);
	if (!p) {
...
void VL53L0X::writeRegister32Bit(uint8_t reg, uint32_t value) {
	...
	bool p = I2Cdev::writeBytes(dev, this->address, reg, 4, data);
	if (!p) {
...
void VL53L0X::writeRegisterMultiple(uint8_t reg, const uint8_t* source, uint8_t count) {
	...
	bool p = I2Cdev::writeBytes(dev, this->address, reg, count, data);
	if (!p) {
...
uint8_t VL53L0X::readRegister(uint8_t reg) {
	uint8_t data;
	int8_t p = I2Cdev::readByte(dev, this->address, reg, &data);
	if (p == -1) {
...
uint16_t VL53L0X::readRegister16Bit(uint8_t reg) {
	...
	int8_t p = I2Cdev::readBytes(dev, this->address, reg, 2, data);
...
uint32_t VL53L0X::readRegister32Bit(uint8_t reg) {
	...
	int8_t p = I2Cdev::readBytes(dev, this->address, reg, 4, data);
...
void VL53L0X::readRegisterMultiple(uint8_t reg, uint8_t* destination, uint8_t count) {
	uint8_t data[count];
	int8_t p = I2Cdev::readBytes(dev, this->address, reg, count, data);
