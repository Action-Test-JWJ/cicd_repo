...
VL6180::VL6180(std::string device_dev_path)
    : device_dev_path(device_dev_path)
{
}
...
int VL6180::read_byte(vl6180 handle, int reg){
    char data_write[2];
    char data_read[1];
...
int VL6180::read_two_bytes(vl6180 handle, int reg){
    char data_write[2];
    char data_read[2];
    data_write[0] = (reg >> 8) & 0xFF; // MSB of register address
    data_write[1] = reg & 0xFF; // LSB of register address
...
void VL6180::write_byte(vl6180 handle, int reg, uint8_t data) {
    char data_write[3];
    data_write[0] = (reg >> 8) & 0xFF;; // MSB of register address
    data_write[1] = reg & 0xFF; // LSB of register address
...
void VL6180::write_two_bytes(vl6180 handle, int reg,int data) {
    char data_write[4];
    data_write[0] = (reg >> 8) & 0xFF;; // MSB of register address
    data_write[1] = reg & 0xFF; // LSB of register address
...
void VL6180::start_range(vl6180 handle) {
    write_byte(handle, 0x018,0x01);
}

void VL6180::poll_range(vl6180 handle) {
    char status;
    char range_status;
...
void VL6180::clear_interrupts(vl6180 handle) {
    write_byte(handle,0x015,0x07);
}

void VL6180::set_scaling(vl6180 handle, int new_scaling){
...
vl6180 VL6180::vl6180_initialise()
{
	return vl6180_initialise_address(VL6180_DEFAULT_ADDR);
}

vl6180 VL6180::vl6180_initialise_address(int addr){
    vl6180 handle=-1;
    std::string buf;
    buf = device_dev_path;
    if ((handle = open(buf.c_str(), O_RDWR)) < 0) {
        return -2;
    }
...
void VL6180::vl6180_change_addr(vl6180 handle, int newAddr)
{
    write_byte(handle, VL6180X_I2C_SLAVE_DEVICE_ADDRESS, newAddr);
    ioctl(handle, I2C_SLAVE, newAddr);
}

int VL6180::RangingData(vl6180 handle){
...
float VL6180::get_ambient_light(vl6180 handle, vl6180x_als_gain_t vl6180x_als_gain) {
