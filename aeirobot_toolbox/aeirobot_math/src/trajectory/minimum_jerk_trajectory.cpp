#include "aeirobot_math/trajectory/minimum_jerk_trajectory.hpp"
                           ...
                           std::vector<double_t> fin_pos, std::vector<double_t> fin_vel, std::vector<double_t> fin_acc)
  {
    ...
    if (fin_time_ > ini_time_)
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        Eigen::Matrix<double, 6, 1> position_coeff = time_mat.inverse() * conditions_mat;
  ...
  std::vector<double_t> MinimumJerk::getPosition(double time)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        double cur_pos = (time_variables_ * position_coeff_.block(0, i, 6, 1)).coeff(0, 0);
        cur_pos_[i] = cur_pos;
      }
    }
    return cur_pos_;
  ...
  std::vector<double_t> MinimumJerk::getVelocity(double time)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        double cur_vel = (time_variables_ * velocity_coeff_.block(0, i, 6, 1)).coeff(0, 0);
        cur_vel_[i] = cur_vel;
      }
    }
    return cur_vel_;
  ...
  std::vector<double_t> MinimumJerk::getAcceleration(double time)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < number_of_joint_; i++)
      {
        ...
        double cur_acc = (time_variables_ * acceleration_coeff_.block(0, i, 6, 1)).coeff(0, 0);
        cur_acc_[i] = cur_acc;
      }
    }
    return cur_acc_;
